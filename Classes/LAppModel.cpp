...
{
	if(LAppDefine::DEBUG_LOG)CCLog("delete model");
...
void LAppModel::load(const char dir[],const char fileName[])
{
    ...
    if(LAppDefine::DEBUG_LOG)CCLog( "create model : %s",fileName);
...
void LAppModel::preloadMotionGroup(const char group[],bool relativePath)
{
    ...
    for (int i = 0; i < len; i++)
	{
        ...
        if(LAppDefine::DEBUG_LOG)CCLog("load motion name:%s ",name.c_str());
...
void LAppModel::releaseMotionGroup(const char group[],bool relativePath)
{
    int len = modelSetting->getMotionNum( group );
    for (int i = 0; i < len; i++)
	{
...
int LAppModel::startMotion(const char group[],int no,int priority)
{
	...
	{
		if(LAppDefine::DEBUG_LOG)CCLog("can't start motion.\n");
		return -1;
	}
    ...
    if(LAppDefine::DEBUG_LOG)CCLog("start motion ( %s : %d )",group,no);
...
void LAppModel::setExpression(const char expressionID[])
{
	...
	if(LAppDefine::DEBUG_LOG)CCLog( "expression[%s]" , expressionID ) ;
	if( motion != NULL )
	{
	...
	{
		if(LAppDefine::DEBUG_LOG)CCLog( "expression[%s] is null " , expressionID ) ;
	}
}
