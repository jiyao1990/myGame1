...
void TouchManager::touchesMoved(cocos2d::CCPoint touches[],int num)
{
	...
	{
        cocos2d::CCPoint pp1 = touches[i1] ;
        
        for( int i2 = 0 ; i2 < num ; i2++ )
		{
            if( i1 == i2 ) continue ;
            
            cocos2d::CCPoint pp2 = touches[i2] ;
    ...
    cocos2d::CCPoint pt1 = touches[index1] ;
    cocos2d::CCPoint pt2 = touches[index2] ;
...
void TouchManager::touchesMoved(float x1, float y1, float x2, float y2) {
    ...
    if( lastTouchDist > 0 )
	{
        scale = powf( dist / lastTouchDist , 0.75 ) ;
