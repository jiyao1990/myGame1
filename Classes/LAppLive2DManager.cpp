...
{
    if(LAppDefine::DEBUG_LOG)CCLog( "tapEvent");
...
void LAppLive2DManager::onUpdate()
{
	...
	CCSize window=director->getWinSize();
	projection.scale(0.5, window.width/window.height * 0.5);
...
{
	sceneIndex=index;
	if(LAppDefine::DEBUG_LOG)CCLog("model index : %d",sceneIndex );
	switch (sceneIndex)
	{
		case SCENE_INDEX_HARU:
			releaseModel();
			models.push_back(new LAppModel());
			models[0]->load(MODEL_KELEQIU_DIR, MODEL_KELEQIU) ;
			break;
		...
		case SCENE_INDEX_MULTI_MODEL:
			releaseModel();
			models.push_back(new LAppModel());
			models[0]->load(MODEL_KELEQIU_DIR, MODEL_HARU_A) ;

			models.push_back(new LAppModel());
			models[1]->load(MODEL_KELEQIU_DIR, MODEL_HARU_B) ;
			break;
		default:
			break;
	}
